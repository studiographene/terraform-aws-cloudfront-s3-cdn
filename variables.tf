...
variable "origin_s3_bucket_accelerate_enabled" {
  description = "Enable S3 bucket accelerate configuration on Origin bucket. Default = false"
  type        = bool
  default     = false
}

variable "default_origin_bucket_custom_name" {
  description = "Use this only on exceptional cases. A custom name for Origin bucket created in this module. If not specified name will be generated by the module."
  type        = string
  default     = null
}
